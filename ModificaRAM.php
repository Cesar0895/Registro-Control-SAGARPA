...
session_start();
    ...
    $row = $resultado->fetch_array(MYSQLI_ASSOC);
        ...
        $RAM = $_GET['RAM'];
        if ($RAM!=null) {
            $sqlram= "UPDATE `memoria_ram` SET `Memoria_RAM`='$RAM' WHERE Id_Memoria='$id_RAM'";
            $mysqli->query($sqlram);

            if ($RAM=1) {
				echo'<script type="text/javascript">
						alert("Tarea Guardada");
						window.location.href="vistaRAM.php"	;
						</script>';
                //header("location:vistaRAM.php");
            }
        }
        

		$sql = "SELECT * FROM `memoria_ram` WHERE  Id_Memoria='$id_RAM'";
		...
		<div class="card">
			<form>
				...
				<div class="form-inline">
					...
					<input type="hidden" class="form-control" id="ram" name="Id_Memoria" value="<?php echo $row['Id_Memoria'] ?>" require>

					<input type="double" class="form-control" id="ram" name="RAM" value="<?php echo $row['Memoria_RAM'] ?>" require>
					<label class="float-right">GB</label>
				</div>
			...
			<a href="vistaRAM.php" class="btn btn-success">Ver lista de memoria RAM registradas</a>
